# =============================================================================
# Version: DIAG-REORG-StageCDiag-v2.2 | Abbr: SC-DIAG
...
def run_stage_c_diagnostics(
    ...
    has_loss_comps    = "nll_loss"        in optional_present   # [v2.2]
    ...
    # P_loss_components: Per-component hybrid loss over epochs [v2.2]
...
def _plot_p_loss_components(
    ...
    """P_loss_components: Per-component hybrid loss over Stage C epochs.

    Plots NLL, consistency, SW2 transport, calibration as separate lines.
    Requires CSMF-MAIN v1.3.29+ epoch_logs keys. Non-fatal if absent.
    """
    if not has_loss_comps:
        logger.warning(
            "SC-DIAG | P_loss_components skipped — loss component keys absent "
            "(requires CSMF-MAIN v1.3.29+)"
        )
        return
    try:
        data = {}
        for key, label in [
            ...
            if vals:
                data[label] = vals
        if not data:
            logger.warning("SC-DIAG | P_loss_components: all component lists empty")
            return
        plot_epoch_lines(
            data_dict   = data,
            output_path = os.path.join(output_dir, "P_loss_components.png"),
            title       = "Stage C — Hybrid Loss Components per Epoch",
            ylabel      = "Loss",
        )
    except Exception as e:
        logger.error(f"SC-DIAG | P_loss_components failed: {e}")
